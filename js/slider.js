...
const effectLevelLine =
  window.previewOverlay.querySelector(`.effect-level__line`);
window.effectLevelValue = window.previewOverlay.querySelector(
  `input[name=effect-level]`
);
window.effectLevelPin =
  window.previewOverlay.querySelector(`.effect-level__pin`);
window.effectLevelDepth =
  window.previewOverlay.querySelector(`.effect-level__depth`);
...
const onEffectLevelLineMouseDown = (e) => {
  ...
  let onMouseMove = (moveEvt) => {
    ...
    window.effectLevelValue.setAttribute(
      `value`,
    ...
    switch (window.imagePreview.classList[1]) {
      case `effects__preview--chrome`:
        window.imagePreview.style.filter = `grayscale(${
          window.effectLevelValue.value / DEFAULT_VALUE
        })`;
        break;
      case `effects__preview--sepia`:
        window.imagePreview.style.filter = `sepia(${
          window.effectLevelValue.value / DEFAULT_VALUE
        })`;
        break;
      ...
      case `effects__preview--phobos`:
        window.imagePreview.style.filter = `blur(${
          window.effectLevelValue.value * 0.03
        }px)`;
        break;
      case `effects__preview--heat`:
        window.imagePreview.style.filter = `brightness(${
          1 + window.effectLevelValue.value * 0.02
        })`;
        break;
      default:
